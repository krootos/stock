...
const Upload = (props) => {
  ...
  const defaultVariations = props.product_details.variations?.map(({ id, variation, price, file_path }) => ({ variation: variation, file: "", file_path: file_path, price: price, id: id }));

  const initialState = {
    uuid: props.product_details.uuid,
    title: props.product_details.title,
    description: props.product_details.description,
    tags: props.product_details.product_tags?.map(x => x.tag_id),
    attribution_required: props.product_details.attribution_required ?? 0,
    accessibility: props.product_details.accessibility ?? 2,
    product_type_id: props.product_details.product_type_id,
    product_category_id: props.product_details.product_category_id,
    tax_id: props.product_details.tax_id ?? "",
    file_types: props.product_details.file_types ?? "",
    thumbnail_image: "",
    main_file: "",
    use_this_photo: props.product_details.use_this_photo ?? 1,
    resources: defaultVariations
  };

  const defaultTags = props.product_details.product_tags?.map(({ tag_id }) => {
    let id, value;
    props.upload_info.tags?.map((tag) => {
      if (tag.id === tag_id) {
        id = tag.name
        value = tag.id
      }
  ...
  const tagOptions = props.upload_info.tags?.map(({ id, name }) => ({ value: id, label: name }));
  ...
  const handleSubmit = (values, actions) => {
    ...
    if (values.accessibility == 2) {
      ...
      values.resources.forEach(function (value) {
        ...
        formData.append("variation_id[]", value.id ?? "")
        formData.append("prices[]", value.price)
        formData.append("variations[]", value.variation)
      });
    }
    props.myProductEditAction(formData, actions);
  };

  useEffect(() => {
    if (props.product_details.product_type_id) {
      props.getProductTypeCategoryAction(props.product_details.product_type_id);
    }
                      ...
                      <div className="upload__content">
                        ...
                        >
                          ...
                          {values.thumbnail_image ? (
                            ...
                            <img
                              src={
                                props.product_details.thumbnail_image
                              }
                    ...
                    {values.accessibility == 2 ? (
                      <>
                        ...
                        <div className="input__group mb-20">
                          ...
                          {values.resources[0].file_path && (
                            <a className="text-end text-link" target="_blank" href={values.resources[0].file_path}>{t("Download File")}</a>
                          )}
                        </div>
                      </>
                    ) : (
                            ...
                            <Fragment>
                              {values.resources.map((lan, index) => (
                                <div className="align-items-center row">
                                  ...
                                  <div className="col-lg-4 p-1 d-none">
                                    <div className="">
                                      <input
                                        type="hidden"
                                        value={values.resources[index].id}
                                  ...
                                  <div className="col-lg-1">
                                    {values.resources.length > 1 && index != 0 && !values.resources[index].id ? (
                                  ...
                                  {values.resources[index].file_path && (
                                    <a className="text-end text-link" target="_blank" href={values.resources[index].file_path}>{t("Download File")}</a>
                                  )}
                                </div>
                              ))}
                    ...
                    {values.accessibility == 2 && (
                      <div className="form-check form-switch mb-3">
                        <input
                          ...
                          type="checkbox"
                          role="switch"
                          checked={values.attribution_required ? 'checked' : ''}
                          id="attribution"
                          onChange={(e) =>
                            setFieldValue(
...
export const getServerSideProps = async ({ req, res, query }) => {
  ...
  return {
    props: {
      ...
      product_details: rules_data.data.data.product,
